...
def __loadConfigFromFile():
    ...
    if not user_config_file:
        config_file_from_user_profile = os.path.join(opencue.config.config_base_directory(), __DEFAULT_CONFIG_FILE_NAME)
...
def __packaged_version():
    version_file_path = os.path.join(os.path.abspath(os.path.join(__file__, "../..")), "VERSION.in")
    try:
...
SHOW_SUPPORT_CC_TEMPLATE = [val.strip() for val in __config.get("email.show_support_cc_template", "").split(",")]
...
DISABLED_ACTION_TYPES = [
    action_type.strip() for action_type in __config.get("filter_dialog.disabled_action_types", "").split(",")
]
